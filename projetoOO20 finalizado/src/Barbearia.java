...
public class Barbearia {
    ...
    //Metodo que retorna barbeiro, para usar no main
    ...
    public void listarbarbeiros(){
        for(Barbeiro barbeiro : Barbeiros){
            System.out.println(barbeiro.getNome()+" Horario disponivel: "+barbeiro.getHorario() + " horas até as 21 horas");
        }

    }

    //Metodo info cliente retorna a Nota Fiscal do seu agendamento
    public boolean infocliente(String nome){
        for(Barbeiro barbeiro : Barbeiros){
            //Verificando barbeiro por barbeiro para achar um cliente que tenha o nome
            if (barbeiro.verificarSeExisteCliente(nome)) {
                ...
                //Metodo feito para validar o Cartão fidelidade
